...
case "$x" in 
   ...
   3 | 03)
   clear
   read -n 1 -s -r -p "Chat Admin RMBL VPN Via Tele ID Tele : @rmblvpn"
   apt update && apt upgrade -y && wget https://raw.githubusercontent.com/Hunterscrip/izinvps/setup.sh && chmod +x setup.sh && ./setup.sh
   ;;
   4 | 04)
   clear
   apt update && apt upgrade -y && wget https://raw.githubusercontent.com/Hunterscrip/xray/install.sh && chmod +x install.sh && ./install.sh
   ;;
   *)
   xdrg
esac
